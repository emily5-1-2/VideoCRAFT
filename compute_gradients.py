...
def _estimate_autodiv(
    ...
    for _ in range(n_samples):
        if noise_type == "rademacher":
            noise = torch.randint_like(outputs, low=0, high=2)
            noise = noise.to(dtype=outputs.dtype) * 2 - 1.0
...
def compute_gradx(fn, x, *args, create_graph=True, retain_graph=False, **kwargs):
    requires_grad = x.requires_grad
    with torch.set_grad_enabled(True):
        x.requires_grad_(True)
        outputs = fn(x, *args, **kwargs)
