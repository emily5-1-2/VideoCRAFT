...
def _batch_inference(model, dataset, batch_size=128, resize=None, device='cuda'):
  ...
  with torch.no_grad():
    for i in start_ids:
      ...
      if resize:
        x = torch.nn.functional.interpolate(x.view(-1, 24, 56, 56), size=resize, mode='bilinear', align_corners=False)
        x = x.view(-1, 3, 8, resize, resize)
...
class Craft(BaseConceptExtractor):
    ...
    def fit(self, inputs: np.ndarray):
        ...
        patches = torch.nn.functional.unfold(inputs.view(-1, 24, vid_frame_size, vid_frame_size), kernel_size=self.patch_size, stride=strides)
        patches = patches.transpose(1, 2).contiguous().view(-1, 3, 8, self.patch_size, self.patch_size)

        # encode the patches and obtain the activations
        activations = _batch_inference(self.input_to_latent, patches, self.batch_size, vid_frame_size,
                                       device=self.device)

        assert torch.min(activations) >= 0.0, "Activations must be positive."

        if len(activations.shape) == 5:
            activations = torch.mean(activations, dim=(2, 3, 4))
    ...
    def transform(self, inputs: np.ndarray, activations: Optional[np.ndarray] = None):
        ...
        num_frames = activations.shape[2]
        activations = activations.permute(0, 2, 3, 4, 1)
        ...
        # (N * H * W * F, C) -> (N, F, H, W, R)
        U = np.reshape(U, (-1, num_frames, activation_size, activation_size, U.shape[-1]))
    ...
    def estimate_importance(self, inputs, class_id, nb_design=32):
        ...
        if len(U.shape) == 2:
            ...
            for u in U:
                u_perturbated = u[None, :] * masks[:, None, None, None, :]
                a_perturbated = np.reshape(u_perturbated,(-1, u.shape[-1])) @ self.W
                a_perturbated = np.reshape(a_perturbated, (len(masks), U.shape[1], U.shape[2], U.shape[3], -1))
                a_perturbated = np.moveaxis(a_perturbated, -1, 1)
